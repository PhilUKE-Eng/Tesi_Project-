...
train_loader, val_loader, test_loader, num_classes = create_dataloaders(dataset_image, batch_size=32, fau_root=dataset_FAU, is_test=False)
...
save_dir = "/home/filippo/Progetto_Tesi/MaterialeTesi/Progetto/ProgettoTesi3.0/Dataset/Modelli/Modello_resnet50FAU/train"
...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        epochs_without_improvement = 0
        model_save_path = os.path.join(save_dir, f'modelresnet50FAU_epoch{epoch+1}.pth')
    ...
    cm_save_path = os.path.join(save_dir, f'confusion_matrix_resnet50FAU_epoch{epoch+1}.png')
    save_confusion_matrix(val_labels, val_preds, num_classes, cm_save_path)
    print(f"Matrice di confusione salvata in {cm_save_path}")

    report_save_path = os.path.join(save_dir, f'classification_report_resnet50FAU_epoch{epoch+1}.txt')
